...
import CurrentList from "@/components/Home/Technical/Current/CurrentList";
	...
	return (
		<motion.div animate={{ opacity: 1, x: 0 }} className="body md:space-y-15 space-y-5 pt-5 max-w-xs">
			...
			<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }} className="sm:invisible sm:-mt-5 h-0">
				<ScrambleText text="CURRENT" settings={{ speed: 0.6, tick: 10 }} />
			</motion.div>
			{/* header for desktop (shows up immediately) */}
			<ScrambleText text="CURRENT" settings={{ speed: 0.6, tick: 10 }} className="invisible -mt-15 h-0 sm:h-5 sm:visible" />
